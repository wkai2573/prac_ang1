...
export class Official1Component implements AfterViewInit {
	...
	ngAfterViewInit(): void {
		...
		let marqueeDiv = document.querySelector<HTMLElement>(".marquee")!;
		let marqueeUl = document.querySelector<HTMLElement>(".marquee > ul")!;
		let time = (marqueeDiv.offsetHeight + marqueeUl.offsetHeight) / 30;
		marqueeUl.style.animationDuration = time + "s";
		...
		let observer = new IntersectionObserver((entries, observer) => {
			entries.forEach(entry => {
				if (entry.isIntersecting) productDiv.classList.add("animated");
			});
		}, options);
